...
const countNumberOfElements = (arr) => arr.reduce((a,v,i) => i + 1, 0);
...
const returnNames = (arr) => arr.reduce((a,v,i) => [...a, v.name], []);
...
const reversedString = (str) => str.split("").reduce((a, v) => v + a);
...
const countNumberOfChildren = (arr) => arr.reduce((a,v,i) => {
    if(v.children != undefined){ return a + v.children.length; }
    else{ return a; }
}, 0);
...
const calculateAverage = (arr) => arr.reduce((a,v,i) => a += v, 0) / arr.length;
...
const countPrimeNumbers = (arr) => arr.reduce((a,v,i) => {
    if(isPrime(v)){ return a += 1; }
    else{ return a; }
}, 0);
