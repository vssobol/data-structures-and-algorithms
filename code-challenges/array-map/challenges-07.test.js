...
const forEachTwoToThe = (arr) => {
  let powerOf = [];
  arr.forEach(i => powerOf.push(Math.pow(2, i)));
  return powerOf;
};
...
const charCode = (arr) => arr.map(i => i.charCodeAt(0));
...
const extractStats = (arr) => arr.map(i => ({name: i.stat.name, total: (i.baseStat + i.effort)}));
